...
CLQ_permutated_matrix_gen <- function(iternum, filename, df_c){
  ...
  #### CELL_TYPES 
  prior_info_cell_types_list <- "/Users/rohit/Downloads/condition/cell_types_celesta.csv"
  ...
  cell_type_assignment <- cell_type_assignment_file$CELL_TYPE_NUMBER
  ...
  for (r in 1:N_PA){
    ...
    for(k in 1:length(cell_type_num)){
      ...
      if (is.na(count_a)){
        CLQ_array = append(CLQ_array, rep(0,29))
        next
      }
      ### j is Cell type A
...
get_counts = function(filename){
  ...
  df_a = data.frame( cell_type = cell_types, cell_type_assignment = c(1:29)) ### cell type number and the names
...
files <- (Sys.glob("/Users/rohit/Downloads/condition/innpd/*cell_type_assignment.csv"))
...
for (f in files){
  ...
  CLQ_permutated = CLQ_permutated_matrix_gen(iternum=5, 
                                             filename = filename_c,
                                             df_c = count_file)
}
...
significance_matrix_gen = function(iternum,
  ...
  for (cA in 1:length(cell_types)){
    for (cB in 1:length(cell_types)){
      ...
      ### current_row is the row number calculated by the cell types number, ie. row number corresponding to the CLQ of cell type 2 and cell type 5 should be: (2-1)*29 + 5 = 23. 
...
plot_gen = function(iternum, filename, list_of_matrices , CLQ_matrix_original){
  ...
  for (cA in 1:29){
    for (cB in 1:29){
      ...
      png(file= file.path(paste0(sample_to_check  ,"cell_",cA,"_", cB,".png")),width=1000, height=600, res=200 )
      par(cex.axis=1.0)
...
for (f in files){
  ...
  significance_matrices = significance_matrix_gen(iternum=5,
                                                  filename = filename_c,
