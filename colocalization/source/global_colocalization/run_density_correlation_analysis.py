...
def identify_metastatic_lymph_nodes():
    clinical_data = pd.read_csv("../../celesta/input/data/clinical_data.csv")

    lymph_node_data = clinical_data[clinical_data["Site"].str.contains("LN", na = False)]
    lymph_node_positive = lymph_node_data[~lymph_node_data["N"].str.contains("N0", na = False)]

    microarrays = (lymph_node_positive["TMA#"].astype(str) + "_" + lymph_node_positive["TMA_part"].astype(str)).tolist()
...
def compute_stain_correlation(image_path, mode, microarray, core, condition, marker_one, marker_two, 
                                  tile_size, marker_one_threshold, marker_two_threshold):
    image = imread(image_path) # (30, 1440, 1920, 4)

    panel = []
    with open("../../celesta/input/data/channel_names.txt") as file:
        for line in file:
    ...
    binary_marker_one_stain = (marker_one_stain >= (marker_one_threshold * np.max(marker_one_stain))).astype(np.uint8)
    binary_marker_two_stain = (marker_two_stain >= (marker_two_threshold * np.max(marker_two_stain))).astype(np.uint8)
    ...
    rgb_marker_one_stain = cv2.cvtColor((binary_marker_one_stain * 255).astype(np.uint8), cv2.COLOR_GRAY2BGR)
    rgb_marker_two_stain = cv2.cvtColor((binary_marker_two_stain * 255).astype(np.uint8), cv2.COLOR_GRAY2BGR)
    ...
    cv2.imwrite(f"../output/{mode}/{condition}/{microarray}_{core}/{marker_one}_mask_threshold_{marker_one_threshold}.png", rgb_marker_one_stain)
    cv2.imwrite(f"../output/{mode}/{condition}/{microarray}_{core}/{marker_two}_mask_threshold_{marker_two_threshold}.png", rgb_marker_two_stain)
    ...
    for i in range(0, rows, tile_size):
        for j in range(0, columns, tile_size):
            marker_one_tile = binary_marker_one_stain[i : min(i + tile_size, rows), j : min(j + tile_size, columns)]
            marker_two_tile = binary_marker_two_stain[i : min(i + tile_size, rows), j : min(j + tile_size, columns)]

            marker_one_proportion = np.sum(marker_one_tile) / (marker_one_tile.shape[0] * marker_one_tile.shape[1])
            marker_two_proportion = np.sum(marker_two_tile) / (marker_two_tile.shape[0] * marker_two_tile.shape[1])

            marker_one_density_map[i // tile_size, j // tile_size] = marker_one_proportion
            marker_two_density_map[i // tile_size, j // tile_size] = marker_two_proportion
    ...
    fig.savefig(f"../output/{mode}/{condition}/{microarray}_{core}/{marker_one}_{marker_two}_heatmap.pdf")
    plt.close(fig)
    ...
    ax.scatter(flattened_marker_one_density_map, flattened_marker_two_density_map, alpha = 0.5)
    ax.set_xlabel(f"{marker_one} tile proportions")
    ax.set_ylabel(f"{marker_two} tile proportions")

    polynomial = np.poly1d(np.polyfit(flattened_marker_one_density_map, flattened_marker_two_density_map, 1))
    ...
    correlation_coefficient, _ = pearsonr(flattened_marker_one_density_map, flattened_marker_two_density_map)
    ...
    fig.savefig(f"../output/{mode}/{condition}/{microarray}_{core}/{marker_one}_{marker_two}_correlation_scatter.pdf")
    plt.close(fig)

    return correlation_coefficient

def parse_args():
    parser = argparse.ArgumentParser(description = "interface for selecting comparator markers")
...
def main():
    ...
    for file in os.listdir(negative_assignments_directory):
        ...
        if match:
            ...
            image_path = f"../../celesta/input/data/{microarray}/processed_imaging_data/{core}_X01_Y01_Z01.tif"

            negative_correlations.append(compute_stain_correlation(image_path, mode, microarray, core, "node_negative_stain", marker_one, marker_two, 
                                                                       tile_size, marker_one_threshold, marker_two_threshold))
    ...
    for file in os.listdir(positive_assignments_directory):
        ...
        if match:
            ...
            positive_correlations.append(compute_stain_correlation(image_path, mode, microarray, core, "node_positive_stain", marker_one, marker_two, 
                                                                       tile_size, marker_one_threshold, marker_two_threshold))
    ...
    data = pd.DataFrame({
        ...
        "category": ["N+"] * len(positive_correlations) + ["N0"] * len(negative_correlations)
    })
    ...
    sns.boxplot(x = "category", y = "density correlation", data = data, color = "lightgray", showfliers = False, ax = ax)
    sns.stripplot(x = "category", y = "density correlation", data = data, color = "black", jitter = True, size = 7, alpha = 0.7, ax = ax)

    y_max = ax.get_ylim()[1] * 0.95
    ax.text(0.5, y_max, f"$p$-value = {p_value:.4f}", ha = "center", va = "center", fontsize = 12, color = "red")
