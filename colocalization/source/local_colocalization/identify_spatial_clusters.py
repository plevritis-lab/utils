...
def __reduce_data_dimensionality(A_cells, A, color_by, save_path, method = "PCA"):
    ...
    if method == "PCA":
        ...
        reduced_representation = umap.UMAP(n_neighbors = min(A_cells.shape[0] - 1, 15)).fit_transform(A_cells)
...
def __cluster_cell_type_embeddings(A_cells, A, save_path, method = "k_means"):
    ...
    if method == "k_means":
        ...
        ax_one.axvline(x = optimal_k, color = "red", linestyle = "--", label = f"optimal k = {optimal_k}")

        ax_one.set_xlabel("k")
        ax_one.set_ylabel("silhouette score")

        ax_two.plot(K, squared_errors, marker = "o")
        ax_two.axvline(x = optimal_k, color = "red", linestyle = "--", label = f"optimal k = {optimal_k}")
        ...
        cluster_labels = KMeans(n_clusters = optimal_k, random_state = 42).fit_predict(A_embeddings)
        ...
        line_vector = np.array([n_points - 1, neighbor_distances[-1]]) - np.array([0, neighbor_distances[0]])
        line_vector_norm = line_vector / np.sqrt(np.sum(line_vector ** 2))
        
        vector_from_first = np.vstack([all_points, neighbor_distances]).T - np.array([0, neighbor_distances[0]])
        scalar_product = np.sum(vector_from_first * np.tile(line_vector_norm, (n_points, 1)), axis = 1)
        vector_from_line = vector_from_first - np.outer(scalar_product, line_vector_norm)
        ...
        ax.axhline(y = optimal_epsilon, color = "r", linestyle = "--", label = f"optimal epsilon = {optimal_epsilon:.3f}")
        ax.scatter(optimal_index, optimal_epsilon, color = "red", marker = "o", label = "elbow point")
...
def __save_spatial_clusters(A_cells, A, save_path):
    ...
    for sample_name, sample_A_cells in A_cells.groupby("SAMPLE"):
        ...
        for cluster_label in sorted(sample_A_cells["CLUSTER"].unique()):
            ...
            plt.scatter(cluster_points["X"], cluster_points["Y"], s = 20, label = f"{A} cluster {cluster_label}")
    ...
    summary_statistics.to_csv(os.path.join(cluster_summaries, f"{A}_cluster_summary.csv"))
...
def cluster_spatial_embeddings(condition, log_normalization = True):
    ...
    if log_normalization:
        ...
        cell_embedding_matrix[normalize_columns] = np.log1p(cell_embedding_matrix[normalize_columns])
