...
compute_cell_counts <- function(files, save_path, cell_types) {
    ...
    count_matrix <- Reduce(function(x, y) {
        dplyr::full_join(x, y, by = "FINAL_CELL_TYPE")
    }, count_matrix)
...
compute_global_colocalization <- function(files, save_path, n_neighbors = 15, bandwidth = 100, weight_scheme = "linear", log_transform = FALSE) {
    colocalization_matrix <- lapply(files, function(file) {
        ...
        sample_assignments <- read.csv(file) %>%
            dplyr::rename("x" = .data$X, y = .data$Y, cell_type = .data$FINAL_CELL_TYPE) |>
            dplyr::mutate(sample = sample_name)

        spomic <- metadisco:::createSpomic(sample_assignments) |>
            metadisco:::setSpomicHypers(tile_size = 250, window_size = 10, k_neigh = n_neighbors,
                                        bandwidth = bandwidth, n_bootstrap = 100, weight_scheme = weight_scheme)

        colocalization <- metadisco:::getCLQs(spomic) |>
            dplyr::mutate(!!sample_name := if (log_transform) log(.data$colocalization_stat + 1) else .data$colocalization_stat) |>
            dplyr::select(.data$A_B, !!sample_name)

        colocalization
    })

    colocalization_matrix <- Reduce(function(x, y) {
        dplyr::full_join(x, y, by = "A_B")
    }, colocalization_matrix)

    transformation <- if (log_transform) "transformed" else "untransformed"
    write.csv(colocalization_matrix, sprintf("%s_global_colocalization_matrix_%s_%s_weighting_%d_neighbors_%d_pixel_bandwidth.csv",
                                             save_path, transformation, weight_scheme, n_neighbors, bandwidth), row.names = FALSE)
}
...
parse_arguments <- function() {
    parser <- argparse::ArgumentParser(description = "batch global CLQ processing of cell assignment files; \
