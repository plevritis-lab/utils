...
def compute_local_colocation_quotient(assignments, A, B, bandwidth, log_transform):
    ...
    distances = cdist(
        A_cells[["X", "Y"]].values, assignments[["X", "Y"]].values
    )  # N_A x N
    ...
    numerator = (
        gaussian_weights.dot(mask)
        - gaussian_weights[np.arange(N_A), A_cells.index] * mask[A_cells.index]
    )
    denominator = (
        gaussian_weights.sum(axis=1) - gaussian_weights[np.arange(N_A), A_cells.index]
    )
...
def generate_spatial_cell_embedding_matrix(
    assignments, save_path, sample_name, cell_types, log_transform, bandwidth
):
    ...
    unique_cell_types = [
        t
        for t in (["unknown"] + cell_types)
        if t in assignments["FINAL_CELL_TYPE"].unique()
    ]
    ...
    for i, A in enumerate(unique_cell_types):
        ...
        A_embeddings = {
            B: compute_local_colocation_quotient(
                assignments, A, B, bandwidth, log_transform
            )
            for B in (["unknown"] + cell_types)
        }
    ...
    composite_embedding_matrix = pd.concat(
        composite_embedding_matrix, ignore_index=True
    )
    composite_embedding_matrix.to_csv(
        os.path.join(
            save_path,
            f"{sample_name}_local_colocalization_matrix_{transformation}_{bandwidth}_pixel_bandwidth.csv",
        ),
        index=False,
        na_rep="nan",
    )
...
def visualize_bandwidth_parameter(centroids, bandwidth, save_path):
    ...
    spatial_neighborhood = plt.Circle(
...
def plot_image_pairwise_colocalizations(
    cell_embedding_matrix, A, B, save_path, cell_type_colors, bins=4
):
    ...
    try:
        cuts = cuts.cat.rename_categories(
            lambda x: pd.Interval(
                abs(max(round(x.left, 2), 0)), round(x.right, 2), closed=x.closed
            )
        )
    except Exception:
        pass
    ...
    if A != B:
        ax.scatter(
            ...
            s=20,
            label=B,
            color=cell_type_colors[B]["color"],
        )
    ...
    fig.savefig(
        os.path.join(save_path, f"{B}_{A}.png"),
        dpi=300,
        bbox_inches="tight",
        pad_inches=0.2,
    )
...
def driver(
    ...
    colocalizations_directory = os.path.join(
        save_path, "colocalizations", "local_colocalizations"
    )

    sample_assignments = glob.glob(
        os.path.join(assignments_directory, "*_assignments.csv")
    )
    if filter != "all":
        sample_assignments = [
            f
            for f in sample_assignments
            if os.path.basename(f)
            in [f"{name}_assignments.csv" for name in filter.split(",")]
        ]
    ...
    for sample_path in sample_assignments:
        sample_name = re.match(
            r"(.+?)_assignments", os.path.basename(sample_path)
        ).group(1)
        ...
        sample_visualizations_directory = os.path.join(
            save_path, "visualizations", "cell_colocalizations", sample_name
        )
        os.makedirs(sample_visualizations_directory, exist_ok=True)

        cell_embedding_matrix = generate_spatial_cell_embedding_matrix(
            assignments,
            colocalizations_directory,
            sample_name,
            signature_matrix["CELL_TYPE"].tolist(),
            log_transform,
            bandwidth,
        )
        ...
        for B in unique_cell_types:
            for A in unique_cell_types:
                plot_image_pairwise_colocalizations(
                    cell_embedding_matrix,
                    A,
                    B,
                    sample_visualizations_directory,
                    cell_type_colors,
                )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="interface for generating LCLQ embeddings"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="bandwidth radius of circular neighborhood to consider; \
                                                                                      defaults to 100",
    )
    parser.add_argument(
        "-c",
    ...
    parser.add_argument(
        "-d",
        "--data_directory",
        help="path to a data directory of cell assignment .csv files",
    )
    parser.add_argument(
        "-f",
        "--filter",
        default="all",
        help="comma-separated list of sample names to process, or 'all' to process everything; \
                                                                   defaults to 'all'",
    )
    parser.add_argument(
        "-m", "--signature_matrix", help="path to a signature matrix file"
    )
    parser.add_argument(
        "-s",
        "--save_path",
        help="path to save LCLQ embeddings and visualizations under colocalizations/local_colocalizations and visualizations/cell_colocalizations, respectively",
    )

    parser.add_argument(
        "--log_transform",
        action="store_true",
        help="whether to log-transform the colocalization statistics",
    )
...
def main():
    ...
    os.makedirs(save_path, exist_ok=True)
    os.makedirs(
        os.path.join(save_path, "colocalizations", "local_colocalizations"),
        exist_ok=True,
    )
    os.makedirs(
        os.path.join(save_path, "visualizations", "cell_colocalizations"), exist_ok=True
    )
