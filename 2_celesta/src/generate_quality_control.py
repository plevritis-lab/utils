...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="interface for generating quality control spreadsheets"
    )

    parser.add_argument(
        "-d",
        "--clinical_data",
        help="path to a clinical data spreadsheet that contains columns 'TMA', 'TMA_PART', and 'CORE_IMAGE_ID'",
    )
    parser.add_argument(
        "-p",
        "--panel_path",
        help="path that points to the underlying location of the channel_names.txt file",
    )
    parser.add_argument(
        "-s",
        "--save_path",
        help="path where the generated quality control spreadsheet will be saved; \
