...
def stratify_marker_probabilities(assignments):
    ...
    probability_columns = assignments.columns[
        assignments.columns.str.endswith("_PROBABILITY")
    ]
    assignments[probability_columns] = assignments[probability_columns].apply(
        lambda x: x.map(categorize_expression)
    )
...
def create_cell_type_outline(assignments, segmentation_mask, cell_type):
    ...
    cell_ids = assignments.loc[
        assignments["FINAL_CELL_TYPE"] == cell_type, "CELL_IDENTIFIER"
    ]
        ...
        if modality == "proteomic":
            for i, protein in enumerate(panel):
                ...
                if protein in markers_to_colors:
                    image_layer.colormap = napari.utils.colormaps.Colormap(
                        ["black", markers_to_colors[protein]]
                    )

        elif modality == "histology":
            image_layer = viewer.add_image(
                np.transpose(spatial_images[modality], (1, 2, 0)),
                name=modality,
                contrast_limits=[0, 1],
                blending="additive",
            )
...
def add_cell_type_visualization(viewer, assignments, colormap, segmentation_mask):
    ...
    if segmentation_mask is not None:
        for cell_type in unique_cell_types:
            ...
            cell_type_outline = create_cell_type_outline(
                assignments, segmentation_mask, cell_type
            )
        ...
        for cell_type in unique_cell_types:
            ...
            centroids = assignments.loc[
                (assignments["FINAL_CELL_TYPE"] == cell_type), ["Y", "X"]
            ]

            image_layer = viewer.add_points(
                centroids,
                name=cell_type,
...
def add_probability_visualization(
    viewer, assignments, panel, image_shape, legend_spacing=(50, 200)
):
    ...
    for i, marker in enumerate(panel):
        ...
        for discretization, rgb_color in probability_color_mapping.items():
            color_sequence[start_index:end_index][
                marker_probabilities == discretization
            ] = rgb_color

        title_index = num_centroids * num_markers + i

        point_cloud_sequence[title_index] = [
            i,
            image_shape[0] / 2,
        ...
        for j, (probability_label, color) in enumerate(
            probability_color_mapping.items()
        ):
            legend_end_index = legend_start_index + j
            vertical_offset = legend_spacing[0] + (j * legend_spacing[0])

            point_cloud_sequence[legend_end_index] = [
                i,
                image_shape[0] / 2 + vertical_offset,
                image_shape[1] + legend_spacing[1],
            ]
...
def create_assignment_overlays(
    spatial_images, assignments, colormap, segmentation_mask=None
):
    ...
    panel = assignments.columns[
        ~assignments.columns.str.contains("_PROBABILITY$")
        & ~assignments.columns.isin(exclude_columns)
    ].tolist()

    markers_to_colors = {
        info["marker"]: info["color"]
        for info in colormap.values()
        if info.get("marker")
    }
    ...
    add_probability_visualization(
        viewer,
        stratify_marker_probabilities(assignments),
        panel,
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="interface for dynamic celesta assignment visualization through napari"
    )
    ...
    parser.add_argument(
        "-i",
        "--image_path",
        help="file path that points to the underlying location of the proteomic TIF or TIFF file",
    )

    parser.add_argument(
        "-e",
        "--histology_path",
        help="(optional) file path that points to the underlying location of the histology TIF or TIFF file",
    )
    parser.add_argument(
        "-m",
        "--mask_path",
        help="(optional) file path that points to the underlying location of the segmentation mask, stored as a .npy file",
    )
...
def main():
    ...
    spatial_images = (
        {"proteomic": imread(image_path), "histology": imread(histology_path)}
        if histology_path
    ...
    create_assignment_overlays(
        spatial_images, assignments, cell_type_info, segmentation_mask
    )
