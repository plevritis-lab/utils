...
def plot_persistence_diagram_density(persistence, A, save_path):
    ...
    persistence = [
        (item["dimension"], (item["birth"], item["death"])) for item in persistence
    ]
    ...
    fig.savefig(
        os.path.join(save_path, f"{A}_persistence_diagram_density.png"), dpi=300
    )
    plt.close(fig)
...
def compute_topological_features(
    assignments, save_path, sample_name, cell_types, count_criteria
):
    ...
    for A in unique_cell_types:
        ...
        if len(A_cells) < count_criteria:
            ...
            persistence_data = [{"dimension": None, "birth": None, "death": None}]

        else:
            persistence = featurize_cell_type_topology(A_cells)

            persistence_data = [
                {"dimension": dimension, "birth": birth, "death": death}
    ...
    pd.DataFrame(
        [
            {"cell_type": A, "persistence": composite_persistence_matrix[A]}
            for A in composite_persistence_matrix
        ]
    ).to_json(output_path, orient="records", indent=2)
...
def driver(assignments_directory, signature_matrix, save_path, filter, count_criteria):
    ...
    for sample_path in sample_assignments:
        ...
        sample_visualizations_directory = os.path.join(
            save_path, "visualizations", "persistence_diagrams", sample_name
        )
        os.makedirs(sample_visualizations_directory, exist_ok=True)

        persistent_homology_features = compute_topological_features(
            assignments,
            topology_directory,
            sample_name,
        ...
        for A in unique_cell_types:
            if len(assignments[assignments["FINAL_CELL_TYPE"] == A]) >= count_criteria:
                plot_persistence_diagram_density(
                    persistent_homology_features[A], A, sample_visualizations_directory
                )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="interface for applying persistent homology to 2D spatial point patterns"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="minimum number of points required to compute persistent homology for a cell type; \
                                                                                  defaults to 3",
    )
    parser.add_argument(
        "-d",
    ...
    parser.add_argument(
        "-s",
        "--save_path",
        help="path to save persistent homology features and diagrams under topology and visualizations/persistence_diagrams, respectively",
    )
...
def main():
    ...
    os.makedirs(
        os.path.join(save_path, "visualizations", "persistence_diagrams"), exist_ok=True
    )
