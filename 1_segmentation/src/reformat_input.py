...
def condense_channels(image_path, panel_path, channels_to_remove):
    ...
    save_directory = os.path.join(os.path.dirname(image_path), "condensed_images")
    save_path = os.path.join(save_directory, sample_name)

    os.makedirs(save_directory, exist_ok=True)
    imwrite(save_path, image)

    condensed_panel_path = os.path.join(
        os.path.dirname(panel_path), "condensed_channel_names.txt"
    )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="interface for formatting input directories prior to applying mesmer or cellpose"
    )
    ...
    parser.add_argument(
        ...
        "--data_directory",
        type=str,
        help="path to the directory containing the images to be segmented; \
...
def reformat_image(image_path):
    ...
    if "Z" in axes:
        ...
        image = np.moveaxis(
            image, [axes.index("C"), axes.index("Y"), axes.index("X")], [0, 1, 2]
        )
