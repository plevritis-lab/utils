...
def compress_channels(images, nuclear_channel, segment_channel, save_path):
    ...
    for i, image in enumerate(images):
        ...
        compressed_image[1, :, :] = image[
            segment_channel, :, :
        ]  # color the segment channel G
        compressed_image[2, :, :] = image[
            nuclear_channel, :, :
        ]  # color the nuclear channel B
...
def apply_mesmer(compressed_images, segment_channel_name, save_path):
    ...
    os.environ.update(
        {"DEEPCELL_ACCESS_TOKEN": "HcI12JDz.jBnsD080yqdai9s5f0LHZodVdEUpPimh"}
    )
    ...
    for i, compressed_image in enumerate(compressed_images):
        ...
        reshaped_image = reshaped_image[
            ..., ::-1
        ]  # swap G (segment channel) and B (nuclear channel)
        ...
        np.save(
            os.path.join(save_path, f"image_{i + 1}_seg.npy"), segmentation_predictions
        )


def overlay_masks(
    compressed_images,
    cellpose_segment_channel_name,
    mesmer_segment_channel_name,
    display_segment_channel_name,
    load_path,
    save_path,
):
    ...
    for i, compressed_image in enumerate(compressed_images):
        ...
        overlaid_image[1, :, :][yellow_mask] = np.iinfo(
            overlaid_image.dtype
        ).max  # color cellpose mask Y
        overlaid_image[0, :, :][red_mask] = np.iinfo(
            overlaid_image.dtype
        ).max  # color mesmer mask R
        ...
        imwrite(
            os.path.join(
                save_path,
                f"image_{i + 1}_cellpose_{cellpose_segment_channel_name}_mesmer_{mesmer_segment_channel_name}_display_{display_segment_channel_name}.tif",
            ),
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="interface for applying cellular segmentation algorithms, mesmer and cellpose; \
    ...
    parser.add_argument(
        "-n",
        "--nuclear_channel",
        help="name of the nuclear channel to be used in segmentation OR a display channel if overlay_masks is toggled",
    )
    parser.add_argument(
        "-s",
        "--segment_channel",
        help="name of the cytoplasm or membrane channel(s) to be used in segmentation OR a display channel if overlay_masks is toggled; \
    ...
    parser.add_argument(
        "--apply_cellpose",
        action="store_true",
        help="segments the image using cellpose",
    )
    parser.add_argument(
        "--apply_mesmer", action="store_true", help="segments the image using memser"
    )
    parser.add_argument(
        "--overlay_masks",
        action="store_true",
        help="overlays the segmentation masks produced by both cellpose and mesmer onto the original image for visual inspection",
    )

    parser.add_argument(
        "-c",
        "--cellpose_channel",
        help="name of the protein channel used by cellpose to generate the image segmentation mask; \
                                                            only relevant if overlay_masks is toggled",
    )
    parser.add_argument(
        "-m",
        "--mesmer_channel",
        help="name of the protein channel used by mesmer to generate the image segmentation mask; \
                                                          only relevant if overlay_masks is toggled",
    )
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="selects and segments certain regions of the image with enough marker coverage for performance testing",
    )
...
def main():
    ...
    if arguments.debug:
        ...
        patches = randomly_sample_patches(
            image,
            nuclear_channel_index,
            segment_channel_index,
        ...
        compressed_images = compress_channels(
            patches,
            nuclear_channel_index,
            (segment_channel, segment_channel_index),
            save_path,
        )
